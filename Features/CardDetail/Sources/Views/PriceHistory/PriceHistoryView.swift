...
struct PriceHistoryView: View {
  ...
  private var chartContent: some View {
    // One chart for every status, so landing prices update it in place instead of building a new
    // Swift Charts hierarchy and crossfading it over the loading one.
    PriceHistoryChart(
    ...
    // The marks and axes jump to the loaded prices, rather than Swift Charts animating them frame
    // by frame on the main thread.
    .transaction(value: display.status) { $0.animation = nil }
    .overlay {
